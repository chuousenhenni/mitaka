 ...
 *===========================================================================================
 */
int ChkSendCom3(void);/* bufferに残ったデータの送信処理 & 送信残りbyte数を確認する */
void SendCom3(HANDLE h, unsigned char *p, int size);/* 制御機間通信(USART_ID_3 1200bps ODD)への送信処理 */
void RcvCom3Init(void);/* 変数等の初期化 */
void RcvCom3Srv(void);/* UART3の受信データを処理してMONITOR_QUEUEに積みなおす処理　メインループ用 */
void TimerIntL2_monitor(int count);/* 受信データ途切れタイマー処理 */
...
//					printf("*** uart3_rx_hunt flag1\n");
			...
			case ST_UART_BODY_NORMAL:
				switch (rx_char) {
					case UART_FLAG_CHAR:
						// ここにCRC checkを追加
						if (len_rcvbuf3() >= 12) {/* kaji20170313 最低長をheader+CRCで12byteとする */
